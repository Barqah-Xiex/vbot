...
async function message(sock, m, store) {
    ...
    if(!(isAdmin||isVIP)) return nyarios(`kamu bukan Admin / VIP`);
    if(!fs.existsSync(dbdir(`togglewelcome`)+id)){
        fs.save(dbdir(`togglewelcome`)+id,`true`)
        nyarios(`berhasil di nonaktifkan`)
    }else{
        fs.unlinkSync(dbdir(`togglewelcome`)+id)
