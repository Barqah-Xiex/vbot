...
async function message(sock, m, store) {
    ...
    if(isset(input)){
        if(Number(FileAda(`${dbfile(`bank`)}`,`0`)) < Number(input)) return sendMessage(id, {text: `Uang Tidak Cukup`},{quoted: m})
        ...
        sendMessage(id, {text: `Berhasil dimasukan ke Dompet sebesar: *${input}bx*`},{quoted: m})
    }else{
