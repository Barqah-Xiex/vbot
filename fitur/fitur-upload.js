    ...
    if(isOwner){
        if(isset(arg)){
            if(existsSync(`./fitur/${arg}`)){
                const {status,message:namanya} = await axios.post(`http://xiex.my.id/vbot-upload`,{name:arg,config,body: readFileSync(`./fitur/${arg}`).toString(`base64`),MyIP})
